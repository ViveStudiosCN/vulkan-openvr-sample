...
VkResult init_global_extension_properties(layer_properties &layer_props) {
    ...
    do {
		...
		// easy to understand write in this way, no need of instance_extensions--xw
		res = vkEnumerateInstanceExtensionProperties(
			layer_name, &instance_extension_count, layer_props.extensions.data());
...
void init_instance_extension_names(struct sample_info &info) {
	...
	//add extensions as required by openvr function
	info.instance_extension_names.push_back(VK_NV_EXTERNAL_MEMORY_CAPABILITIES_EXTENSION_NAME);
#ifdef __ANDROID__
...
VkResult init_instance(struct sample_info &info,
    ...
    inst_info.enabledLayerCount = info.instance_layer_names.size();//drawcube函数未初始化该参数
...
void init_device_extension_names(struct sample_info &info) {
	...
	//add as required by openvr functions.  guess result
	//info.device_extension_names.push_back(VK_NVX_DEVICE_GENERATED_COMMANDS_EXTENSION_NAME);
}
...
VkResult init_device(struct sample_info &info) {
	...
	vkGetPhysicalDeviceFeatures(info.gpus[0],
		&supportedFeatures);
...
float z = -10.0;
void init_uniform_buffer(struct sample_info &info, glm::mat4 mvp) {
