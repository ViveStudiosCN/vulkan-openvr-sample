...
int sample_main(int argc, char *argv[]) {
	...
	//leftEye.m_nImage = (uint64_t)bltSrcImage;
	leftEye.m_nImage = (uint64_t)info.buffers[info.current_buffer].image;
	...
	//rightEye.m_nImage = (uint64_t)bltSrcImage;
	rightEye.m_nImage = (uint64_t)info.buffers[info.current_buffer].image;
	rightEye.m_pDevice = info.device;
	rightEye.m_pPhysicalDevice = info.gpus[0];
	rightEye.m_pInstance = info.inst;
	rightEye.m_pQueue = info.graphics_queue;
	rightEye.m_nQueueFamilyIndex = info.graphics_queue_family_index;
	rightEye.m_nWidth = info.width;
	rightEye.m_nHeight = info.height;
	//rightEye.m_nWidth = 1520;
	//rightEye.m_nHeight = 1680;
	rightEye.m_nFormat = VK_FORMAT_R8G8B8A8_UNORM;
	...
	//buffersize2 = vr::VRCompositor()->GetVulkanDeviceExtensionsRequired(r.GetVulkanPhysicalDevice(), NULL, 0);
	//char* pValue2 = (char*)malloc(buffersize2);
	//vr::VRCompositor()->GetVulkanDeviceExtensionsRequired(r.GetVulkanPhysicalDevice(), pValue2, buffersize2);
	...
	//const vr::VRTextureBounds_t bounds2 = { .5f, 0.f, 1.f, 1.f };
	vr::VRCompositor()->Submit(vr::Eye_Right, &rightEyeTexture);
	
	vr::TrackedDevicePose_t temp[vr::k_unMaxTrackedDeviceCount];
	vr::VRCompositor()->WaitGetPoses(temp, vr::k_unMaxTrackedDeviceCount, NULL, 0);
